...
export interface ClientEntry {
  ...
  /** Path under /public, e.g. /non-entertainment/dhl.png */
  logo?: string;
}

/** A roster entry with its work resolved, as the clients page renders it. */
export type ClientCard = ClientEntry & {
  slug: string;
  work: { slug: string; title: string }[];
};

/** URL-safe id for a client — the `?client=` value on /case-studies. */
export const clientSlug = (name: string) =>
  name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');

/**
 * Same brand, different name on the case study. Only for names no rule can
 * connect — everything else is matched below.
 */
const ALIASES: Record<string, string[]> = {
  Protinex: ['ProteinX India'],
  Dabur: ['Vatika Nourishing Oils'],
};

const words = (s: string) => s.toLowerCase().match(/[a-z0-9]+/g) ?? [];

/**
 * Is `client` the brand behind `clientName`? True when the client's words
 * appear as a whole-word run inside it, so "Zydus" claims "Zydus Vaxiflu" and
 * "Zydus Lifesciences", "Sandu" claims "Sandu Pharma", and one study named
 * "DHL x Mumbai Indians" belongs to both DHL and Mumbai Indians. Matching on
 * words rather than substrings keeps "Fiat" off "FiatPe".
 */
function claims(client: string, clientName: string) {
  const a = words(client);
  const b = words(clientName);
  if (!a.length || !b.length) return false;
  return b.some((_, i) => a.every((w, j) => b[i + j] === w));
}

const claimsWithAliases = (client: string, clientName: string) =>
  claims(client, clientName) || (ALIASES[client] ?? []).some((alias) => claims(alias, clientName));

/**
 * A client's case studies, newest first (the order `studies` already arrives
 * in). Derived from each study's `clientName` rather than a stored relation:
 * the studio already asks for the client on every case study, so a new one
 * links itself with nothing extra to fill in.
 */
export const studiesForClient = (client: string, studies: CaseStudyGalleryItem[]) =>
  studies.filter((s) => claimsWithAliases(client, s.clientName ?? ''));

/** The roster client a case study belongs to, if it is one of ours. */
export const clientForStudy = (study: CaseStudyGalleryItem) =>
  CLIENT_ROSTER.find((c) => claimsWithAliases(c.name, study.clientName ?? ''));

/**
 * The rest of the work for this study's client. Resolving through the roster
 * first is what groups "Zydus Vaxiflu" with "Zydus Lifesciences" — the two
 * names share no words, only a brand.
 */
export function siblingStudies(study: CaseStudyGalleryItem, studies: CaseStudyGalleryItem[]) {
  const client = clientForStudy(study);
  const family = client
    ? studiesForClient(client.name, studies)
    : studies.filter((s) => s.clientName === study.clientName);
  return { client, studies: family.filter((s) => s.slug !== study.slug) };
}
