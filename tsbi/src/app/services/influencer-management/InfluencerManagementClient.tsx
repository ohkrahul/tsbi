...
/* ─── Page ──────────────────────────────────────────── */
export default function InfluencerManagementClient({ campaigns }: { campaigns: Campaign[] }) {
        ...
        <section style={{ background: 'var(--navy)', padding: '80px 48px' }}>
          <div style={{ maxWidth: 1300, margin: '0 auto' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, marginBottom: 32 }}>
              <h2 style={{ fontFamily: 'var(--fm)', fontSize: 'clamp(26px,3.4vw,44px)', fontWeight: 600, color: '#fff', margin: 0, lineHeight: 1.1 }}>
                Selected Work
              </h2>
              <Link href="/case-studies" style={{ fontFamily: 'var(--fm)', fontSize: 12, letterSpacing: '.06em', color: MAGENTA, textDecoration: 'none' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>
              {[...campaigns].sort(newestDayFirst).map((c) => (
                <Link
                  ...
                  style={{ display: 'block', textDecoration: 'none', borderRadius: 12, overflow: 'hidden', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)' }}
                >
                  {c.poster && (
                    /* eslint-disable-next-line @next/next/no-img-element */
                    <img src={c.poster} alt={c.title} style={{ display: 'block', width: '100%', aspectRatio: '16/9', objectFit: 'cover' }} />
                  )}
                  <div style={{ padding: '14px 16px 16px' }}>
                    <div style={{ fontFamily: 'var(--fm)', fontSize: 9, letterSpacing: '.12em', textTransform: 'uppercase', color: MAGENTA, marginBottom: 6 }}>
                      {c.client}
                    </div>
                    <div style={{ fontFamily: 'var(--fd)', fontSize: 15, fontWeight: 700, color: '#fff', lineHeight: 1.25 }}>
                      {c.title}
                    </div>
                  </div>
                </Link>
